...
chroms <- c("A01" ,"A02" ,"A03", "A04","A05","A06" ,"A07", "A08","A09","A10","C01", "C02", "C03", "C04","C05", "C06", "C07", "C08", "C09")
for (i in chroms) {
...
# Make rqtl recombination fraction plot

# Note that decreasing point size will decrease label size
...
finalmap <- drop.markers(mymapcor10_chr, bad_rf_markers)
save(finalmap, file="mymap_cor10_p23_chr_cor.RData") 
...
##########################
# The below commented code was executed per chromosome on a HPC
#load("mymap_cor10_p23_chr_cor.RData")
#i <- c("A01")
...
chroms <- names(finalmap$geno)
for (i in chroms) {
  # Get df of marker names and positions
  chr_order <- pull.map(finalmap, chr=i)
...
plot(ntyped(finalmap), ylab="No. typed markers", main="No. genotypes by individual")
plot(ntyped(finalmap, "mar"), ylab="No. typed individuals",main="No. genotypes by marker")
dev.off()
...
clonecheck <- genClones(finalmap, tol = 0.9, id = "ID")
sink('finalmap_clonecheck.txt')
clonecheck$cgd
sink()
...
map_tmp<- subset(finalmap, ind=c("129", "150", "107","156"))
pdf("four_sample_test.pdf")
geno.image(map_tmp,chr = "C09")
dev.off()
...
for (i in chroms) {
  ...
  myxo <- locateXO(finalmap, chr = i, full.info=TRUE)
...
write.csv(all_chr,file = "Pop3_ddrad_finalmap_locatexo_all_chr.csv", row.names=FALSE,quote=FALSE)
# Markers flanking a crossover can be assigned physical positions using this table
write.csv(all_mark,file = "Pop3_ddrad_finalmap_marker_positions.csv", row.names=FALSE,quote=FALSE)
...
write.cross(finalmap, filestem = "finalmap_p23_cor_hetcleanGC_dropbadrfmarkers", format = "csvsr")
# The finalmap cross is encoded as bcsft 2 with 0 backcross generation
# This was recommended for F2 in ASMap manual
